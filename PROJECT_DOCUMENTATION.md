...
- Расширение плагина users-permissions (`src/extensions/users-permissions/strapi-server.js`) добавляет публичные маршруты:
  ...
  - `POST /api/phone-auth/confirm` — тело `{ phone, code }`; берёт последний неиспользованный OTP, проверяет срок; помечает used; находит роль `authenticated`; создаёт пользователя (username=phone, phone, random password, confirmed=true, роль) или переносит его в роль authenticated; выпускает JWT через `plugin::users-permissions.jwt.issue`; возвращает `{ jwt, user }` без приватных полей.
...
- Действия: `blocked=true`, `deletedAt=now`, `phone=null`, `email/username` заменяются на уникальные `deleted_{id}`; очищаются `resetPasswordToken`, `confirmationToken`, `provider`; все адреса пользователя удаляются; в заказах пользователя обнуляются `phone`, `shipping_address`, `comment`, `details` (связь `user` остаётся).
