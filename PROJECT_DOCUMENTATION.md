...
- Прод-образ: Node 22-alpine, ставит build-deps для `canvas`, копирует проект, `npm run build`, затем `npm start`.
...
- **Пользователь (users-permissions, расширение)**: добавлены `phone` (string, unique, regex `+?[0-9]{10,15}`), связи oneToMany ↔ addresses/orders. Email и username отмечены как required/unique.
